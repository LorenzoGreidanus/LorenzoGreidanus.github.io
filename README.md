...
| `server/beheer.js` | Meldingen bij vragen ("Klopt deze vraag niet?") en de gebruikstelling per dag per pagina (Durable Object); te lezen op `leermiddelen/beheer.html` met de beheersleutel |
...
| `leermiddelen/bank.js` | De vragenbank: vakken, niveaus, de vragen per vak (NED, ENG, GES, AARD en de vervolgen), de onderdelen en de rekengenerator, en onderaan de lijsten van de Werkwoordrace, Irregular verbs en Tijdvakken sorteren; Vlaggen en Landenvormen lezen AARD (met `n3` als hun eigen niveau). Gedeeld door alle spellen die vragen stellen; vragen pas je hier aan (bovenin staat per vak een lijst, met ernaast een lijst met het niveau 1 tot 4 per vraag). Alles staat er met `var`, zodat een spel zijn eigen NIVEAUS erbovenop mag zetten |
...
| `leermiddelen/strijd.js` | Samen spelen vanuit Torenverdediging en Zwaardvechter: het potje met vrienden (Zwaardvechter tot vier in een arena, de maker start of het begint vanzelf bij vier; Torenverdediging voor twee, begint vanzelf), de klasstrijd (`?kamer=CODE`, haakjes `STRIJD.klaar`, `STRIJD.reeks`, `STRIJD.af`) en het klassement van de site op start- en eindscherm. Samen met een vriend is echt samen, in de pas (lockstep): allebei rekenen precies hetzelfde spel uit. In Zwaardvechter in vaste stappen van een zestigste seconde, met de toetsen, het ontwijken en het klaar-zijn in de winkel als opdrachten die de gastheer per stap bevestigt (`STRIJD.stuurNet`, haakje `net`), en de maat die met een kleine voorraad stappen achter hem aan loopt en zijn tempo iets bijstelt om die voorraad te houden; de klok van de gastheer tikt vanuit een Web Worker, zodat hij doorrekent als zijn tabblad niet zichtbaar is. In Torenverdediging net zo: allebei rekenen precies hetzelfde spel uit, tik voor tik, met hetzelfde toeval (`zaai`/`toeval`, het zaad komt van de gastheer), en alles wat een speler doet (bouwen, versterken, verplaatsen, een slot kopen, een ronde vervroegen, een antwoord) is een opdracht die bij allebei in dezelfde tik wordt uitgevoerd (`voerUit`); de gastheer bevestigt om de twee tikken welke opdrachten erin zaten, met af en toe een vingerafdruk van de stand, en de maat loopt een paar tikken achter hem aan. Klopt de vingerafdruk niet meer (verschil in rekenwerk tussen browsers), dan valt het terug op het doorsturen van een compacte stand acht keer per seconde |
| `leermiddelen/race.html` | Vragenrace: zestig seconden zoveel mogelijk vragen goed uit de gedeelde bank, voor Nederlands, Engels, geschiedenis en aardrijkskunde; meldt bij een klascode |
| `leermiddelen/lesbrieven.html` | Per spel een lesbrief voor collega's (leerdoel, tijd, niveau, werkvorm, nabespreking), te filteren en af te drukken |
| `leermiddelen/klas.html` | Klasoverzicht voor de docent: maak een klascode (geldig tot de docent hem opheft, of ruim een jaar zonder gebruik), leerlingen koppelen zich in de leeromgeving met code en bijnaam, en na elk potje Torenverdediging of Zwaardvechter staat de uitslag hier, per leerling en als tabel, te downloaden als CSV; de sleutel staat alleen in de browser van de docent |
| `leermiddelen/beheer.html` | Voor de beheerder: meldingen bij vragen nalopen, het gebruik per pagina zien en rijen uit het klassement halen; vraagt de beheersleutel (secret `BEHEER`) |
| `leermiddelen/melding.js` | Het knopje "Klopt deze vraag niet?" onder een vraag (`MELDING.knop`, `MELDING.volg`), in Torenverdediging, Zwaardvechter en op het bord van de Klasquiz |
| `server/profiel.js` | De speelcode (Durable Object `Profiel`, binding PROFIEL, migratie v6): per code van acht letters een profiel zonder persoonsgegevens (avatar, beste per spel, campagne, vrijgespeeld, klascode, niveau); `/maak`, `/lees`, `/sync` (samenvoegen: nieuwste record wint), weg na 400 dagen zonder gebruik. Routes in index.js: `POST /api/profiel`, `GET/PUT /api/profiel/CODE` |
| `server/account.js` | Inloggen met Microsoft, optioneel (Durable Object `Account`, binding ACCOUNT, migratie v7): OpenID Connect met PKCE tegen Entra ID (`common`: school en persoonlijk), het id_token wordt nagekeken (handtekening via JWKS, aud, iss, exp, nonce), per account alleen een hash van `sub`, de weergavenaam en de gekoppelde speelcode; sessie in een getekend HttpOnly-koekje (90 dagen). Routes `GET /api/account`, `GET /api/account/inloggen`, `GET /api/account/terug`, `POST /api/account/koppel`, `POST /api/account/uitloggen`, `DELETE /api/account`. Staat pas aan met de secrets `MS_CLIENT_ID`, `MS_CLIENT_SECRET` en `SESSIE_GEHEIM` (zie onder) |
| `leermiddelen/profiel.js` | De speelcode aan de kant van de leerling (`PROFIEL.code/avatar/zetAvatar/maak/koppel/sync`, en `account/accountAfstemmen/inlogAdres/uitloggen/accountWeg` voor het optionele Microsoft-account): verzamelt `lg-beste-*`, `lg-toren-campagne`, `lg-toren-*` vrij, `lg-klas` en `lg-niveau` uit localStorage en zet wat van de server komt weer terug; de kiezer voor de eigen avatar staat in de leeromgeving |
| `leermiddelen/tekenslag.html` | Tekenslag: een leerling tekent een begrip op zijn laptop, het bord tekent live mee, zijn team raadt; teams tegen de klok, woorden uit de vragenbank of eigen lijst (rollenkamer, game `teken`) |
| `leermiddelen/rollen.js` en `rol.html` | Rollenspellen op telefoons: het bord opent een kamer (`ROLSPEL.maak/host`), verdeelt rollen (`ROLSPEL.verdeel`) en stuurt kaarten naar telefoons (`naar`, `kaarten`, `alle`); een delegatie kiest met `ROLSPEL.delegatie` (meeste stemmen). `rol.html` is de ene spelerspagina die rolkaarten, stemmingen, latten en lijsten tekent en de keuze terugstuurt. Gebruikt door vergaderklas, meetlat, vergadering, berlijn en grieken |
| `leermiddelen/avatar.js` | Een avatar per bijnaam (`AVATAR.svg(naam, maat)`): een blob in de vorm van het logo-op-hover uit het merkboek, merkkleur en gezichtje uit een hash van de naam, dus overal dezelfde; met een spec (`v3k2o1m0e4`, gekozen in de leeromgeving) telt de naam niet meer. De spec reist mee als `av` bij klasmeldingen en bij het verbinden met een kamer. Staat bij elke naam met een score: klassementen, Klasquiz, Klasstrijd, klasoverzicht, eindkaart |
| `leermiddelen/spel.js` en `spel.css` | Het gedeelde skelet van elk spel: het uitlegpaneel op het startscherm (`SPEL.uitleg`, doel, tijd en bediening, in te klappen) en de eindkaart (`SPEL.einde`, score, sterren, beste ooit op dit apparaat, delen, klasmelding). Elk spel roept die twee aan; de eigen inhoud blijft in het spel zelf |
| `leermiddelen/beeld/` | Drie schermafbeeldingen voor het blok voor collega's op de spellenpagina |
| `leermiddelen/klas.js` | De koppeling met de klas aan de kant van de leerling (`KLAS.lees/zet/wis/meld`); de spellen melden hun einduitslag via `/api/klas/CODE/meld`; een docent meldt een hele uitslag ineens (Klasquiz) via `/api/klas/CODE/melden` met de sleutel van de klas |
...
- Een klascode is een kamer zonder spel: leerlingen melden er uitslagen (hoogstens dertig per leerling per spel, drieduizend per klas, vierhonderd meldingen per minuut per adres), alleen de docent leest ze met de sleutel; de code blijft tot de docent hem opheft (`POST /api/klas/CODE/opheffen` met de sleutel) of tot hij 400 dagen niet gebruikt is. Wie met Microsoft is ingelogd kan zijn klaskoppeling niet zelf losmaken.
...
- De poortwachter telt per adres, en een hele school zit achter één adres: de grenzen zijn daarom ruim (negentig kamers per tien minuten, negentig scores per twee minuten, negenhonderd profielsyncs per minuut, driehonderd Microsoft-logins per tien minuten). Het gaat om het afremmen van een stroom, niet om een klas.
...
- Inloggen met Microsoft aanzetten: maak op portal.azure.com (Microsoft Entra ID, App registrations) een registratie met accounttype "Accounts in any organizational directory and personal Microsoft accounts", platform Web, omleidings-URI `https://meneergreidanus.nl/api/account/terug`; maak onder Certificates & secrets een clientgeheim (hoogstens 24 maanden, daarna vernieuwen). Zet dan `npx wrangler secret put MS_CLIENT_ID` (de Application (client) ID), `npx wrangler secret put MS_CLIENT_SECRET` en `npx wrangler secret put SESSIE_GEHEIM` (een lange willekeurige tekst). Zonder die drie is de knop onzichtbaar. Lokaal testen kan met `--var MS_AANMELDBASIS:http://127.0.0.1:8793` tegen een nagemaakte aanmeldserver.
