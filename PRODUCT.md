...
- Vierendertig spellen (met de dagelijkse uitdaging en Oefen je fouten), 2856 vragen in één gedeelde vragenbank (waaronder per vak de examenstof vmbo: geschiedenis, aardrijkskunde, biologie, wiskunde, economie, maatschappijkunde en Engelse examenwoorden), tweeëndertig lesbrieven.
...
- 2856 vragen in `leermiddelen/bank.js` (september 2026: 421 vragen erbij, vooral bij de onderdelen en niveaus die nog dun waren: biologie, wiskunde en samenleving in leerjaar 1-2, en vwo bij Nederlands, Engels en geschiedenis) (negen vakken, met Economie als examenvak); 32 lesbrieven in `leermiddelen/lesbrieven.html`; 34 spellen in de catalogus van `leermiddelen/index.html`. De site is gesplitst in een leerlingkant (`leermiddelen/index.html`: één docentknop, ingeklapte filters, vijf spellen per vak met een meer-knop, een rij Samen op het digibord) en een docentkant (`leermiddelen/klas.html`, ook via /docent: wie-ben-je-balk, kaarten naar handleiding/lesbrieven/Klasquiz/Tekenslag, spellenkeuze en lesmodus per klas). Een klascode maken vereist een Microsoft-sessie (`/api/kamer` weigert spel=klas zonder inlog). De klas bewaart `spellen` en `lesmodus`; `/api/klas/CODE/instelling` zet ze, `/api/klas/CODE/mijn` geeft ze aan de leerling. Dagelijkse uitdaging (`dag.html`): elke dag dezelfde tien vragen voor iedereen (zaad = datum), één poging per apparaat, klassement per dag (`/api/klassement/dag-JJJJ-MM-DD`). Foutenmap (`foutenmap.js`, `fouten.html`): fout beantwoorde vragen uit Vragenrace, Torenverdediging, Zwaardvechter en de dagelijkse uitdaging komen terug tot ze goed gaan; kenmerk en vak reizen mee in het profiel. Weekopdracht: de docent zet bij een klascode een opdracht (spel, vak, onderdeel, minimum, einddatum; `/api/klas/CODE/opdracht`), leerlingen zien een kaart in de leeromgeving (`/api/klas/CODE/mijn`), het klasoverzicht toont wie hem haalde; `klas.html?demo=1` is een demoklas. Dagstreak met muntbonus op elk eindscherm. Torenverdediging heeft een uitlegstand met tips. Gezichtjes ademen, knipperen, volgen de muis en hebben stemmingen (au, blij, sip). `node server/audit-bank.js` toetst de vragenbank. De leeromgeving kent naast het niveau een leerjaar (1-2 of 3-4); in leerjaar 3-4 staat de examenstof van het vmbo erbij: Nederlands (De tekstdetective), geschiedenis, aardrijkskunde, biologie, wiskunde, economie, maatschappijkunde en Engelse examenwoorden. Het klasoverzicht toont per leerling per onderdeel het aandeel goed (formatief). Wie is ingelogd spaart munten (per goed antwoord in elk oefenspel, meer per antwoord in lange spellen) voor cosmetica in de leeromgeving (leermiddelen/cosmetica.js: hoeden, randen, zwaardskins, en trofeeën die je vrijspeelt door in Zwaardvechter een baas te verslaan); het saldo staat op de server in het profiel. Zwaardvechter heeft drie stijlen (ridder, schutter, schildwacht met blokkeren), een critical na drie goed op rij, arenagevaren vanaf ronde 4, elitefouten en een vaardigheidskeuze na elke baas. Tekenslag (`leermiddelen/tekenslag.html`) is een klassikaal tekenspel op de rollenkamer (game teken, bericht vlug voor streken): een leerling tekent op zijn laptop, het bord tekent mee, zijn team raadt. Het Bronnenlab heeft 46 bronnen: 20 echte (twee per tijdvak, met afbeelding), 14 oefenbronnen en 12 vaardigheidsbronnen.
